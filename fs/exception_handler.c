#include "exception_handler.h"

#define CPU_STACK_TRACE_DEPTH		10
#define __stringify(rn)				#rn

#define mfspr(_rn) \
({	register uint32_t _rval = 0; \
	asm volatile("mfspr %0," __stringify(_rn) \
	: "=r" (_rval));\
	_rval; \
})

typedef struct _framerec {
	struct _framerec *up;
	void *lr;
} frame_rec, *frame_rec_t;

static const char * exception_names[] = {
    0,  //SYSTEM_RESET",
    0,  //MACHINE_CHECK",
    "DSI",
    "ISI",
    0,  //"EXTERNAL_INTERRUPT",
    0,  //ALIGNMENT",
    "PROGRAM"
};

static unsigned char exception_cb(OSContext* context, unsigned char exception_type) {
    char buf[850];
    int pos = 0;

    /*
     * This part is mostly from libogc. Thanks to the devs over there.
     */
	pos += __os_snprintf(buf + pos, sizeof(buf) - pos, "Exception type %s occurred!\n", exception_names[exception_type]);
	pos += __os_snprintf(buf + pos, sizeof(buf) - pos, "GPR00 %08X GPR08 %08X GPR16 %08X GPR24 %08X\n",context->gpr[0], context->gpr[8], context->gpr[16], context->gpr[24]);
	pos += __os_snprintf(buf + pos, sizeof(buf) - pos, "GPR01 %08X GPR09 %08X GPR17 %08X GPR25 %08X\n",context->gpr[1], context->gpr[9], context->gpr[17], context->gpr[25]);
	pos += __os_snprintf(buf + pos, sizeof(buf) - pos, "GPR02 %08X GPR10 %08X GPR18 %08X GPR26 %08X\n",context->gpr[2], context->gpr[10], context->gpr[18], context->gpr[26]);
	pos += __os_snprintf(buf + pos, sizeof(buf) - pos, "GPR03 %08X GPR11 %08X GPR19 %08X GPR27 %08X\n",context->gpr[3], context->gpr[11], context->gpr[19], context->gpr[27]);
	pos += __os_snprintf(buf + pos, sizeof(buf) - pos, "GPR04 %08X GPR12 %08X GPR20 %08X GPR28 %08X\n",context->gpr[4], context->gpr[12], context->gpr[20], context->gpr[28]);
	pos += __os_snprintf(buf + pos, sizeof(buf) - pos, "GPR05 %08X GPR13 %08X GPR21 %08X GPR29 %08X\n",context->gpr[5], context->gpr[13], context->gpr[21], context->gpr[29]);
	pos += __os_snprintf(buf + pos, sizeof(buf) - pos, "GPR06 %08X GPR14 %08X GPR22 %08X GPR30 %08X\n",context->gpr[6], context->gpr[14], context->gpr[22], context->gpr[30]);
	pos += __os_snprintf(buf + pos, sizeof(buf) - pos, "GPR07 %08X GPR15 %08X GPR23 %08X GPR31 %08X\n",context->gpr[7], context->gpr[15], context->gpr[23], context->gpr[31]);
	pos += __os_snprintf(buf + pos, sizeof(buf) - pos, "LR    %08X SRR0  %08x SRR1  %08x\n", context->lr, context->srr0, context->srr1);

	if(exception_type == OS_EXCEPTION_DSI) {
        pos += __os_snprintf(buf + pos, sizeof(buf) - pos, "DAR   %08X DSISR %08X\n", context->exception_specific1, context->exception_specific0); // this freezes
	}

    void *pc = (void*)context->srr0;
    void *lr = (void*)context->lr;
    void *r1 = (void*)context->gpr[1];
	register uint32_t i = 0;
	register frame_rec_t l,p = (frame_rec_t)lr;

	l = p;
	p = r1;
	if(!p)
        asm volatile("mr %0,%%r1" : "=r"(p));

	pos += __os_snprintf(buf + pos, sizeof(buf) - pos, "\nSTACK DUMP:");

	for(i = 0; i < CPU_STACK_TRACE_DEPTH-1 && p->up; p = p->up, i++) {
		if(i % 4)
            pos += __os_snprintf(buf + pos, sizeof(buf) - pos, " --> ");
		else {
			if(i > 0)
                pos += __os_snprintf(buf + pos, sizeof(buf) - pos, " -->\n");
			else
                pos += __os_snprintf(buf + pos, sizeof(buf) - pos, "\n");
		}

		switch(i) {
			case 0:
				if(pc)
                    pos += __os_snprintf(buf + pos, sizeof(buf) - pos, "%p",pc);
				break;
			case 1:
				if(!l)
                    l = (frame_rec_t)mfspr(8);
				pos += __os_snprintf(buf + pos, sizeof(buf) - pos, "%p",(void*)l);
				break;
			default:
				pos += __os_snprintf(buf + pos, sizeof(buf) - pos, "%p",(void*)(p->up->lr));
				break;
		}
	}

	if(exception_type == OS_EXCEPTION_DSI) {
		uint32_t *pAdd = (uint32_t*)context->srr0;
		pos += __os_snprintf(buf + pos, sizeof(buf) - pos, "\nCODE DUMP:\n");
		// TODO by Dimok: this was actually be 3 instead of 2 lines in libogc .... but there is just no more space anymore on the screen
		for (i = 0; i < 8; i += 4)
            pos += __os_snprintf(buf + pos, sizeof(buf) - pos, "%p:  %08X %08X %08X %08X\n", &(pAdd[i]),pAdd[i], pAdd[i+1], pAdd[i+2], pAdd[i+3]);
	}

    OSFatal(buf);
    return 1;
}

static unsigned char dsi_exception_cb(OSContext* context) {
    return exception_cb(context, OS_EXCEPTION_DSI);
}
static unsigned char isi_exception_cb(OSContext* context) {
    return exception_cb(context, OS_EXCEPTION_ISI);
}
static unsigned char program_exception_cb(OSContext* context) {
    return exception_cb(context, OS_EXCEPTION_PROGRAM);
}

void setup_os_exceptions(void) {
    OSSetExceptionCallback(OS_EXCEPTION_DSI, &dsi_exception_cb);
    OSSetExceptionCallback(OS_EXCEPTION_ISI, &isi_exception_cb);
    OSSetExceptionCallback(OS_EXCEPTION_PROGRAM, &program_exception_cb);
}
